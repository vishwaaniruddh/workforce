...
$type = 'R';
$membership_number = get_member_details('NewGenerateMember_Id',$member_id);
...
$type = 'C';
$membership_number = get_member_details('GenerateMember_Id',$member_id);
    $booklet = $_row['booklet_Series'];
    $R = date("M-Y", strtotime($_row['ExpiryDate']));
...
if($From=="" && $To==""){
 ...
 // echo "Select Count(MembershipDetails_Level) as level from Members where MembershipDetails_Level='1' and date('entryDate') between '".$FromDaat."' and '".$Todaat."'  ";
  $QcountGold=mysqli_query($conn,"Select Count(MembershipDetails_Level) as level from Members where MembershipDetails_Level='1' and (date(entryDate) between '".$FromDaat."' and '".$Todaat."' or date(CancelationDate) between '".$FromDaat."' and '".$Todaat."') ");
  $QfetchGold=mysqli_fetch_array($QcountGold);
  
  $QcountPlatinum=mysqli_query($conn,"Select Count(MembershipDetails_Level) as level from Members where MembershipDetails_Level='2' and (date(entryDate) between '".$FromDaat."' and '".$Todaat."' or date(CancelationDate) between '".$FromDaat."' and '".$Todaat."') ");
  $QfetchPlatinum=mysqli_fetch_array($QcountPlatinum);
  
  $QcountSilver=mysqli_query($conn,"Select Count(MembershipDetails_Level) as level from Members where MembershipDetails_Level='6' and (date(entryDate) between '".$FromDaat."' and '".$Todaat."' or date(CancelationDate) between '".$FromDaat."' and '".$Todaat."') ");
  $QfetchSilver=mysqli_fetch_array($QcountSilver);
  
  $TotalLevelQuery=$QfetchFirst['0']+$QfetchGold['0']+$QfetchPlatinum['0']+$QfetchSilver['0'];
  
$totallevel = $QfetchGold['0']+$QfetchPlatinum['0']+$QfetchSilver['0'];
...
<tr>
<th>Silver</th><td>'.$QfetchSilver['0'].'</td>
</tr>
<tr style="background-color:gray">
<th>Total</th><td>'.$totallevel.'</td>
</tr>
